...
Application::Application(GLFWwindow* window, const Options& options)
	...
	, m_present(false), m_presentNext(true), m_presentAtSecond(1.0), m_previousComplete(false),
	m_typeLens(TYPE_LENS_PINHOLE), m_samplesSqrt(1), m_epsilonFactor(500.0f), m_clockFactor(1000.0f),
	m_useDirectLighting(true), m_mouseSpeedRatio(10.0f), m_idGroup(0), m_idInstance(0), m_idGeometry(0)
{
	try {
		m_timer.restart();
		...
		const TypeLight typeEnv = (!m_lightsGUI.empty()) ? m_lightsGUI[0].typeLight
			: NUM_LIGHT_TYPES; // NUM_LIGHT_TYPES means not an environment light either.

		m_raytracer = std::make_unique<Raytracer>(m_maskDevices, typeEnv, m_interop, tex, pbo, m_sizeArena,
			m_peerToPeer);
		...
		m_raytracer->initCameras(
			m_cameras);                  // Currently there is only one but this supports arbitrary many which could be used to select viewpoints or do animation (and camera motion blur) in the future.
		...
		m_raytracer->initLights(m_lightsGUI,
			m_materialsGUI); // With arbitrary mesh lights, the geometry attributes and indices can only be filled after initScene().
...
bool Application::render()
{
	...
	try {
		...
		const auto iterationIndex = m_raytracer->render<0>();

		// When the renderer has completed all iterations, change the GUI title bar to green.
		const bool complete = (m_samplesSqrt * m_samplesSqrt <= iterationIndex);

		if (complete) {
			guiRenderingIndicator(false); // Not rendering anymore.
			flush = !m_previousComplete;  // Completion status changed to true.
		}
		...
		if (m_presentAtSecond < seconds || flush || finish) {
			...
			if (flush || finish) [[unlikely]] // Only print the performance when the samples per pixels are reached.
			{
...
void Application::benchmark()
{
	...
	try {
		...
		while (iterationIndex < spp) {
			iterationIndex = m_raytracer->render<1>();
		}
...
void Application::guiWindow()
{
	...
	if (ImGui::CollapsingHeader("System")) {
		...
		if (ImGui::InputInt2("Resolution", &m_resolution.x,
			ImGuiInputTextFlags_EnterReturnsTrue)) // This requires RETURN to apply a new value.
		{
	...
	if (ImGui::CollapsingHeader("Tonemapper")) {
		...
		if (ImGui::DragFloat("White Point", &m_tonemapperGUI.whitePoint, 0.01f, 0.01f, 255.0f, "%.2f",
			2.0f)) // Must not get 0.0f
		{
			changed = true;
		}
	...
	if (ImGui::CollapsingHeader("Materials")) {
		...
		for (int iMaterial = 0; iMaterial < static_cast<int>(m_materialsGUI.size()); ++iMaterial) {
			...
			if (ImGui::TreeNode((void*)(intptr_t)iMaterial, "%s", m_materialsGUI[iMaterial].name.c_str())) {
				if (ImGui::Combo("BxDF Type", (int*)&materialGUI.typeBXDF,
					"BXDF\0BRDF Diffuse\0BRDF Specular\0BTDF Specular\0BSDF Specular\0BRDF GGX Smith\0BTDF GGX Smith\0BSDF GGX Smith\0\0")) {
					changed = true;
					changedBXDF = true;
				}
				...
				if (ImGui::Checkbox("Thin-Walled",
					&materialGUI.thinwalled)) // Set this to true when using cutout opacity!
				{
					changed = true;
				}
				...
				if (materialGUI.typeBXDF == TYPE_BTDF_SPECULAR ||
					...
					if (ImGui::DragFloat("Absorption Scale", &materialGUI.scaleAbsorption, 0.01f, 0.0f, 1000.0f,
						"%.2f")) {
						changed = true;
					}
					...
					if (ImGui::DragFloat("Scattering Scale", &materialGUI.scaleScattering, 0.01f, 0.0f, 1000.0f,
						"%.2f")) {
						changed = true;
					}
				...
				if (materialGUI.typeBXDF == TYPE_BRDF_GGX_SMITH ||
					materialGUI.typeBXDF == TYPE_BTDF_GGX_SMITH ||
					materialGUI.typeBXDF == TYPE_BSDF_GGX_SMITH) {
					if (ImGui::DragFloat2("Roughness", reinterpret_cast<float*>(&materialGUI.roughness), 0.001f, 0.0f,
						1.0f, "%.3f")) {
				...
				if (materialGUI.typeEDF != TYPE_EDF) // The default black EDF has no parameters.
				{
					...
					if (ImGui::DragFloat("Multiplier", &materialGUI.multiplierEmission, 0.01f, 0.0f, 1000000.0f, "%.2f",
						2.0f)) {
						materialGUI.multiplierEmission = dp::math::clamp(materialGUI.multiplierEmission, 0.0f,
							1000000.0f);
						changed = true;
					}
		...
		if (tokenType == PTT_ID) {
			...
			else if (token == "arenaSize") // In mebi-bytes.
			{
				...
				if (m_tileSize.x & (m_tileSize.x - 1)) {
					std::cerr << "ERROR: loadSystemDescription() tileSize.x = " << m_tileSize.x
						<< " is not power-of-two, using 8.\n";
					m_tileSize.x = 8;
				}
				if (m_tileSize.y & (m_tileSize.y - 1)) {
					std::cerr << "ERROR: loadSystemDescription() tileSize.y = " << m_tileSize.y
						<< " is not power-of-two, using 8.\n";
					m_tileSize.y = 8;
				}
			}
				...
				tokenType = parser.getNextToken(token);
				MY_ASSERT(tokenType == PTT_VAL);
				m_walkLength = std::max(1,
					atoi(token.c_str())); // max volume scattering random walk lengths, default and minimum 1.
			}
...
bool Application::saveSystemDescription()
{
	...
	description << "camera " << m_camera.m_phi << " " << m_camera.m_theta << " " << m_camera.m_fov << " "
		<< m_camera.m_distance << '\n';
	...
	const int indexLight)
{
	...
	auto instance = std::make_shared<sg::Instance>(m_idInstance++);
	...
	group->addChild(std::move(instance));
}
		...
		if (tokenType == PTT_ID) {
			...
			case KS_MATERIAL: {
				tokenType = parser.getNextToken(
					cur.material.name); // Internal material reference name. If there are duplicates the last name wins.
			...
			case KS_ROTATE: {
				...
				const dp::math::Mat44f matrix(rotation, dp::math::Vec3f(0.0f, 0.0f,
					0.0f)); // Zero translation to get a Mat44f back.
				...
				const dp::math::Mat44f matrixInv(rotationInv, dp::math::Vec3f(0.0f, 0.0f,
					0.0f)); // Zero translation to get a Mat44f back.
			...
			case KS_MODEL:
				...
				if (token == "plane") {
					...
					m_scene->addChild(std::move(instance));
				}
				else {
			...
			case KS_LIGHT: {
				...
				{
					std::cerr << "ERROR: loadSceneDescription() Light referenced unknown material "
				...
				if (token == "env") // Constant or spherical texture map environment light.
				{
					...
					if (m_lightsGUI.empty()) {
						...
						lightGUI.typeLight = (materialGUI.nameEmission.empty()) ? TYPE_LIGHT_ENV_CONST
						...
						if (!materialGUI.nameEmission.empty()) {
							...
							if (it != m_mapPictures.end()) {
								MY_ASSERT((it->second->getFlags() & IMAGE_FLAG_RECT) ==
									0); // Cannot have the same texture for env and rect. There is only one CDF.
					...
					"rect") // Rectangle area light, supports importance sampled textures. (Mesh lights don't.)
				{
					...
					lightGUI.area = dp::math::length(
						vecU ^ vecV); // Rectangle area is the length of the cross product of the edges.
					...
					"spot") // Spot light singular light type. Emission texture is projected over the open spread angle.
				{
					LightGUI lightGUI;
					...
					if (!materialGUI.nameProfile.empty()) {
						...
						if (it != m_mapPictures.end()) {
							it->second->addFlags(
								IMAGE_FLAG_IES); // FIXME This is redundant because generateIES() sets that already.
						}
					}
					// No geometry. A singular light cannot be hit implicitly.
				}
...
void
Application::calculateTangents(std::vector<TriangleAttributes>& attributes, const std::vector<unsigned int>& indices)
{
